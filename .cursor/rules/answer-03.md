Stage 03 delivered. Realtime agent wired with a single tool `search_kb(query)`, live transcript, and last answer display. Uses the OpenAI Agents SDK for Realtime with WebRTC and a client‑side tool that calls your `/rag/search` endpoint. ([OpenAI GitHub][1])
...
# 2) Upload documents with optional folder. Index immediately.
...
<body class="bg-slate-50 text-slate-900">
  <div class="max-w-3xl mx-auto p-6 space-y-8">
    <header class="space-y-2">
      ...
      <p class="text-sm text-slate-600">WebRTC voice agent + minimal RAG + tool calling.</p>
    </header>
    ...
    <section class="p-4 bg-white rounded-lg border space-y-4">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <h3 class="text-sm font-medium">Live transcript</h3>
          <pre id="transcript" class="text-xs bg-slate-100 p-2 rounded overflow-auto max-h-40 whitespace-pre-wrap"></pre>
        </div>
        <div>
          <h3 class="text-sm font-medium">Last answer</h3>
          <div id="answer" class="text-sm bg-slate-100 p-2 rounded min-h-[2.5rem]"></div>
        </div>
      </div>
  ...
  <!-- Module script to use OpenAI Agents Realtime in the browser -->
...
3. Test.
   ...
   * Click **Start**. Speak. You should hear the agent and see live transcript and last answer.
   * Upload a PDF/DOCX/TXT. Then ask a clinic question. The agent will call `search_kb` and cite sources.
4. Docker (optional).
...
* Realtime WebRTC with ephemeral tokens is the recommended browser pattern. The SDK can accept a custom WebRTC transport for your own `audioElement` and mic stream. ([OpenAI GitHub][1])
* Tools are defined with `tool()` and Zod parameters; the model will auto‑invoke them and your `execute` returns JSON back to the agent. ([OpenAI GitHub][2])
* The transport exposes events like `audio_transcript_delta` and `audio_done` which we attach to update UI text. ([OpenAI GitHub][3])

This completes Stage 03.

[1]: https://openai.github.io/openai-agents-js/guides/voice-agents/transport/ "Realtime Transport Layer | OpenAI Agents SDK"
[2]: https://openai.github.io/openai-agents-js/guides/tools/ "Tools | OpenAI Agents SDK"
[3]: https://openai.github.io/openai-agents-js/openai/agents-realtime/type-aliases/realtimetransporteventtypes/?utm_source=chatgpt.com "RealtimeTransportEventTypes | OpenAI Agents SDK"
